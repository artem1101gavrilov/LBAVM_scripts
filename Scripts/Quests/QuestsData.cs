    ...
    void CreateQuestsList2()
    {
        ...
        foreach (XmlNode item in dataList)
        {
            ...
            foreach (XmlNode itemItens in itemContent)
            {
                ...
                else if (itemItens.Name == "status")
                {
                    if ((Quest.Status)int.Parse(itemItens.InnerText) == Quest.Status.ACTIVE) currentQuestID = newQuest.id;
                    newQuest.status = (Quest.Status)int.Parse(itemItens.InnerText);
                }
